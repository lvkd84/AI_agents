...
import P4Agents.edu.cwru.sepia.agent.planner.actions.*;
...
public class PEAgent extends Agent {
    ...
    public Map<Integer, Action> middleStep(State.StateView stateView, History.HistoryView historyView) {
        // TODO: Implement me!
    		...
    		if (action instanceof Move) {
    			...
    			res.put(nextAction.peasantId, createSepiaAction(nextAction,stateView));
    ...
    private Action createSepiaAction(StripsAction action, State.StateView state) {
    		if (action instanceof Move)
    			return Action.createCompoundMove(((Move) action).peasantId, ((Move) action).to.x, ((Move)action).to.y);
    		else if (action instanceof Deposit) {
    			return Action.createPrimitiveDeposit(((Deposit) action).peasantId, ((Deposit) action).getDirection(state));
    		} else if (action instanceof HarvestGold) {
    			return Action.createPrimitiveGather(((HarvestGold) action).peasantId, ((HarvestGold) action).getDirection(state));
    		} else if (action instanceof HarvestWood) {
    			return Action.createPrimitiveGather(((HarvestWood) action).peasantId, ((HarvestWood) action).getDirection(state));
    		} else
    			return null;
    }
